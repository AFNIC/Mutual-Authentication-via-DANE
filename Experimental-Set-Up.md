...
Focussing on the mutual authentication between Afnic's NS and TSP JS, we tested the validation using self-signed certificates. Since the intermediate certificates (which is used to generate the leafge self-signed certificates) for both the LoRaWANs were different, TLS handshake were emitting error messages. 
